							...
							<div class="row">
								...
								<div class="col-sm-10">
									<div class="form-group label-floating is-empty @error('kode') has-error @enderror">
										...
										<input type="text" class="form-control" name="kode" value="{{$single_soal->kode}}">
									</div>
								</div>
							</div>
							<div class="row">
										...
										<div class="col-lg-5 col-md-6 col-sm-3">
											<div wire:ignore>
												<select class="form-control" data-style="btn btn-primary btn-round" title="Pilih Jenis Soal" data-size="7" wire:model="jenis_soal">
													...
													<option value="pilihan" {{$single_soal->jenis_soal == "pilihan" ? "selected" :""}}>Pilihan Ganda</option>
													<option value="uraian" {{$single_soal->jenis_soal == "uraian" ? "selected" :""}}>Uraian</option>
												</select>
											</div>
										</div>
									</div>
											...
											<div wire:ignore>
												<select class="form-control" data-style="btn btn-primary btn-round" title="Pilih Matpel" data-size="7" wire:model="matpel_id">
													...
													@forelse ($matpels as $matpel)
														<option value="{{$matpel->id}}" {{$single_soal->matpel_id == $matpel->id ? "selected" : ""}}>{{$matpel->matpel}}</option>
													@empty
														{{-- empty expr --}}
													@endforelse
												</select>
